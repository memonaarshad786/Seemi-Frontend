...
export default function LandingHero() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (type === "checkbox") {
      ...
      if (name === "password") {
        if (value && (!strongPasswordRegex.test(value) || value.length < 8)) {
          setErrors((prev) => ({
            ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.password) {
      ...
      nextErrors.password = passwordRequirementMessage;
    }
  ...
  return (
    <section className="bg-[#A7AFFD] overflow-hidden relative">
      <div className="grid lg:grid-cols-2 gap-6 justify-center items-center px-24 py-12">
        ...
        <div className="flex flex-col gap-6 ml-12">
          <div className="flex items-center gap-0 mb-8">
            <img
              src="/assets/1st-comp/Seemi-Logo.png"
              alt="Seemi Logo"
              className="max-w-[150px] object-contain"
            />
            <img
              src="/assets/1st-comp/Vector.png"
              alt="Vector Icon"
              className="w-[149px] h-[100px] ml-auto mr-32"
            />
          </div>

          <p
            className="font-extrabold text-[50px] leading-[55px] text-[#0b2347]"
            style={{ fontFamily: "Nunito" }}
          >
            Everything You Need
            <br />
            To <span className="text-[#ff6b4a]">Manage & Grow</span>
            <br />
            Your Tutoring Business
          </p>

          <p className="text-[0.95rem] leading-6 max-w-[480px] -mt-6 text-[#4a5568]">
            <strong className="text-[#848ac0]">Seemi</strong> streamlines your
            tutoring business with real-time tools for scheduling, progress
            tracking, and payments. Focus on teaching while we handle the rest.
          </p>
        </div>
        ...
        <div className="relative flex flex-col items-start justify-center">
          <div className="bg-[#EAECFF] border-6 border-[#0b2347] rounded-2xl shadow-[0_15px_50px_rgba(0,0,0,0.12)] w-full max-w-[450px] p-8 pb-40 mt-12 relative z-20">
            <h2 className="text-[1.4rem] font-semibold mb-6 text-black">
              Get <span className="text-[#ff6b4a] font-bold">30 days</span>{" "}
              free!
            </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  {errors.confirmPassword && (
                    <span className="text-red-600 text-sm mt-1 block placeholder:text-[#8fa2c4]">
                      {errors.confirmPassword}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-start gap-2 text-[#4a5568] text-sm mt-2">
                ...
                <label htmlFor="terms">
                  ...
                  <a
                    href="#terms"
                    className="text-[#5560d5] font-medium hover:underline"
                  >
                    Terms of Service
                  </a>{" "}
                  and{" "}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-[234px] h-[47px] bg-[#5969FB]  transition-all rounded-full text-white font-semibold mt-1 flex items-center justify-center  disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
      ...
      <img
        src="/assets/1st-comp/Mask%20Group.png"
        alt="Building Icon"
        className="w-full h-[300px] relative -mt-62 pointer-events-none z-20"
      />
    </section>
  );
}
